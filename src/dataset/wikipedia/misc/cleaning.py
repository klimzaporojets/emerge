...
from .recursive_cleaning import recursive_clean, recursive_clean_klim
...
def expand_convert_api(convert_text, request_session):
    ...
    logger.info(f'IT took {time.time() - current_time} seconds for request in {current_process().name}: '
                f' {PARAMS}')
    DATA = R.json()
...
def gross_clean(text, regexes, convert_through_api, convert_to_text_dictionary: DictProxy, request_session1,
    ...
    # 2024.08.07 noise consisting of many repetitive [[[[[[
    text = re.sub(r'\[{20,}', '', text, flags=re.DOTALL)
    ...
    if convert_through_api:
        for curr_found_convert in convert_all_regex.finditer(text):
            ...
            if convert_text not in convert_to_text_dictionary:
                logger.info(f'about to acquire v_lock {current_process().name} to convert: {convert_text}')
                v_lock.acquire()
                logger.info(f'v_lock acquired {current_process().name}')
                # with convert_to_text_dictionary.get_l
                done = False
                nr_retries = 0
                if convert_text not in convert_to_text_dictionary:
                    while not done and nr_retries < 5:
                        ...
                        try:
                            ...
                            logger.error(traceback.format_exc())
                            sleep(5)
                logger.info(f'about to release v_lock {current_process().name}')
                v_lock.release()
                logger.info(f'v_lock released {current_process().name}')
            else:
...
pattern = r'{{Airport codes.*?}}'
pattern_rp = r'{{rp\|.*?}}'
pattern_no_convert = r'{{(?!convert).*?}}'
pattern_convert = r'{{convert\|([^|]+)\|([^|]+)\|([^|]+).*?}}'


# {{convert|2.5|mi|km}}

def clean_random_stuff(text):
    # Define the regex pattern to match {{Airport codes ...}}

    # Use re.sub to remove all matches of the pattern
    # Use re.sub to remove all matches of the pattern
    # cleaned_text = re.sub(pattern, '', text)
    # cleaned_text = re.sub(pattern_rp, '', cleaned_text)
    cleaned_text = re.sub(pattern_no_convert, '', text)

    # Define the regex pattern to match {{convert|...}} with exactly 4 elements

    # Find all matches in the text
    # matches = re.findall(pattern_convert, text)

    def replace_match(match):
        second_element = match.group(1)  # Extract the second element
        third_element = match.group(2)  # Extract the third element
        return second_element + ' ' + third_element  # Concatenate the two elements
        # return second_element + ' ' + third_element + f'--deletemeconvert{match}--'  # Concatenate the two elements

    # Use re.sub with the replacement function
    cleaned_text = re.sub(pattern_convert, replace_match, cleaned_text)

    # Extract the second and third elements from each match
    # extracted_elements = [(match[1], match[2]) for match in matches]
