...
from edc.extract_emerge import ExtractorEmerge
from edc.schema_canonicalization_emerge import SchemaCanonicalizerEmerge
...
class EDC_emerge:
    ...
    def oie(
        ...
        if not llm_utils.is_model_openai(self.oie_llm_name):
            ...
            extractor = ExtractorEmerge(oie_model, oie_tokenizer)
        else:
            extractor = ExtractorEmerge(openai_model=self.oie_llm_name)
        logger.info('extractor_emerge_initialized')
        oie_triples_list = []
        oie_triples_not_in_text_list = []
        ...
        if previous_extracted_triplets_list is not None:
            ...
            for idx, input_text in enumerate(tqdm(input_text_list)):
                ...
                refined_oie_triplets = triples['extracted_triplets_list']
                oie_triples_list.append(refined_oie_triplets)
                oie_triples_not_in_text_list.append(triples['extracted_triplets_not_in_text_list'])
        else:
            # Normal OIE
            ...
            for input_text in tqdm(input_text_list):
                #             'extracted_triplets_list': extracted_triplets_list,
                #             'extracted_triplets_not_in_text_list': extracted_triplets_not_in_text_list
                # logger.info(f'extractor_is: {extractor}, input_text is: {input_text}, '
                #             f'oie_few_shot_examples_str is: {oie_few_shot_examples_str}, '
                #             f'oie_few_shot_prompt_template_str is: {oie_few_shot_prompt_template_str}')
                logger.info('about_to_invoke extractor.extract')
                oie_triples = extractor.extract(input_text, oie_few_shot_examples_str, oie_few_shot_prompt_template_str)
                logger.info(f'after_invoke extractor.extract oie_triples is: {oie_triples}')
                #
                oie_triples_list.append(oie_triples['extracted_triplets_list'])
                oie_triples_not_in_text_list.append(oie_triples['extracted_triplets_not_in_text_list'])
    ...
    def schema_definition(self, input_text_list: List[str], oie_triplets_list: List[List[str]], free_model=False):
        ...
        for idx, oie_triplets in enumerate(tqdm(oie_triplets_list)):
            ...
            print(
                f"***** schema_definition_result: {input_text_list[idx]}, {oie_triplets}\n -> {schema_definition_dict}\n")
    ...
    def schema_canonicalization(
        ...
        print(f'sc_verify_prompt_template_str: {sc_verify_prompt_template_str}')
        ...
        if not llm_utils.is_model_openai(self.sc_llm_name):
            ...
            schema_canonicalizer = SchemaCanonicalizerEmerge(self.schema, sc_embedder,
        ...
        for idx, input_text in enumerate(tqdm(input_text_list)):
            ...
            for oie_triplet in oie_triplets:
                canonicalized_triplet, canon_candidate_dict = schema_canonicalizer.canonicalize(
                    input_text, oie_triplet, sd_dict, sc_verify_prompt_template_str,
                    self.enrich_schema
                )
                if canonicalized_triplet is None:
                    print(f'canonicalized_triplet_in_none with '
                          f'oie_triplet: {oie_triplet}, \n\t'
                          f'canonicalized_triplet: {canonicalized_triplet}, \n\t'
                          f'sd_dict: {sd_dict}, \n\t'
                          # f'sc_verify_prompt_template_str: {sc_verify_prompt_template_str}, \n\t'
                          f'self.enrich_schema: {self.enrich_schema}')
    ...
    def extract_kg(self, input_text_list: List[str],
        ...
        for iteration in range(refinement_iterations + 1):
            ...
            oie_triplets_list, oie_triplets_not_in_text_list, entity_hint_list, \
                ...
                input_text_list,
                # kzaporoj - free the model, so uses less memory
                # free_model=True,
                # kzaporoj - end free the model
                # kzaporoj - old code without freeing the model:
                free_model=self.oie_llm_name not in required_model_dict_current_iteration.values()
                           and iteration == refinement_iterations,
                # kzaporoj - end old code without freeing the model:
                previous_extracted_triplets_list=triplets_from_last_iteration,
            )

            # oie_merged_triples = [(ct) oie_triplets_list]
            # oie_no_action = [[entry[:-1] for entry in sublist] for sublist in oie_triplets_list]
            # oie_no_action = [[entry[:-1] for entry in sublist] for sublist in oie_triplets_list]
            oie_merged_triples = list()

            # def camel_to_words(s):
            #     # Insert a space before each capital letter, then lowercase the whole string
            #     return re.sub(r'(?<!^)(?=[A-Z])', ' ', s).lower()
            for curr_oie_triplets, curr_oie_triplets_not_in_text in zip(oie_triplets_list,
                                                                        oie_triplets_not_in_text_list):
                # curr_oie_triplets = [entry[:-1] for entry in curr_oie_triplets]
                # try:
                new_oie_triplets = []
                ...
                # curr_oie_triplets = [[str(entry[0]).replace('_', ' '),
                #                       entry[1],
                #                       str(entry[2]).replace('_', ' ')] \
                #                      for entry in curr_oie_triplets]
                # curr_oie_triplets_not_in_text = [
                #     [str(entry[0]).replace('_', ' '),
                #      entry[1],
                #      str(entry[2]).replace('_', ' ')] \
                #     for entry in curr_oie_triplets_not_in_text]
            ...
            print(f'sd_dict_list_is: {sd_dict_list}')
            # pdb.set_trace()
            del required_model_dict_current_iteration["sc_embed"]
            del required_model_dict_current_iteration["sc_verify"]

            # logger.info(f'kzaporoj - freeing self.oie_llm_name: {self.oie_llm_name}')
            # llm_utils.free_model(self.loaded_model_dict[self.oie_llm_name][0],
            #                      self.loaded_model_dict[self.oie_llm_name][1])
            # del self.loaded_model_dict[self.oie_llm_name]
            # gc.collect()
            # torch.cuda.empty_cache()
            # torch.cuda.ipc_collect()
            # torch.cuda.synchronize()
            # logger.info(f'kzaporoj - END freeing self.oie_llm_name: {self.oie_llm_name}')
