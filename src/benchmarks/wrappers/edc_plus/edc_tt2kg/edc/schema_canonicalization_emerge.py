...
class SchemaCanonicalizerEmerge:
    ...
    def __init__(
        ...
        # relations_cache_path = f'caches/{relations_cache_path}'
    ...
    def llm_verify(
        ...
        if self.verifier_openai_model is None:
            ...
            verification_result = llm_utils.generate_completion_transformers(
                ...
                answer_prepend="Answer: ", max_new_token=6
            )
        else:
            verification_result = llm_utils.openai_chat_completion(
                self.verifier_openai_model, None, messages, max_tokens=1
            )
        print(f'****** canonicalization_messages: {messages}')
        print(f'****** canonicalization_verification_result: {verification_result}')
        print(f'****** canonicalization_choice_letters_list: {choice_letters_list}')
        if verification_result[0] in choice_letters_list:
            ...
            print(f'****** not_in_choice_letters_list: {verification_result[0]} not_in '
    ...
    def canonicalize(
        ...
        if len(self.schema_dict) != 0:
            if open_relation not in open_relation_definition_dict:
                print('****** open_relation_not_in_open_relation_definition_dicr: '
                      f'triplet: {open_triplet}'
                      f' open_relation: {open_relation} not in {open_relation_definition_dict} ')
